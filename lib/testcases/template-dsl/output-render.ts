...
export function renderOutputValue(
  ...
  if (resolvedValue === undefined) {
    throw new TemplateDslError(`Output references unknown variable "${value.ref}"`);
  }
...
export function renderLinesInstruction(
  ...
  if (!Array.isArray(resolved)) {
    throw new TemplateDslError(`lines.from="${instruction.from}" must resolve to an array`);
  }
