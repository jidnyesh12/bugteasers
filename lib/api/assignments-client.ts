...
export async function fetchAssignmentDetail<TAssignment>(assignmentId: string): Promise<TAssignment> {
  ...
  const parsedBody = await parseJson<ErrorPayload | AssignmentResponse<TAssignment>>(response);
...
export async function unassignAssignmentFromClassroom(
  ...
  const response = await fetch(`/api/assignments/${assignmentId}/assign?classroom_id=${classroomId}`, {
    method: 'DELETE',
  });
...
export async function addProblemToAssignment(assignmentId: string, problemId: string): Promise<void> {
...
function isAssignmentPayload<TAssignment>(value: unknown): value is AssignmentResponse<TAssignment> {
  if (!value || typeof value !== 'object') {
    return false;
  }
