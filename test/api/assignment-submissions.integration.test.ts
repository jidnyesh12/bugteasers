...
describe("GET /api/assignments/[id]/submissions", () => {
  ...
  it("returns 401 when the requester is not authenticated", async () => {
    ...
    const response = await getAssignmentSubmissions(
      new NextRequest("http://localhost:3000/api/assignments/a-1/submissions", {
        method: "GET",
      }),
  ...
  it("returns 403 when instructor does not own the assignment", async () => {
    ...
    const { getAssignmentSubmissionOverview } =
      await import("@/lib/submissions/service");
    ...
    vi.mocked(supabase.from).mockReturnValue(
      lookupChain as unknown as ReturnType<typeof supabase.from>,
    );
  ...
  it("returns assignment submission overview for the assignment owner", async () => {
    ...
    vi.mocked(getAssignmentSubmissionOverview).mockResolvedValue({
      ...
      problems: [{ id: "problem-1", title: "Two Sum", orderIndex: 0 }],
      summaries: [
        {
          studentId: "student-1",
          problemId: "problem-1",
    ...
    expect(data.submissions.summaries[0].selectedSubmission.status).toBe(
      "passed",
    );
