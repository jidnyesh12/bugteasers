...
describe("GET /api/problems/[id]/submissions", () => {
  ...
  it("returns 401 when user is not authenticated", async () => {
    ...
    const request = new NextRequest(
      "http://localhost:3000/api/problems/problem-1/submissions",
      {
        method: "GET",
      },
    );
  ...
  it("returns transformed submission history for authenticated student", async () => {
    ...
    expect(query.order).toHaveBeenCalledWith("submitted_at", {
      ascending: false,
    });
  ...
  it("applies assignmentId filter when query param is provided", async () => {
    ...
    const request = new NextRequest(
      "http://localhost:3000/api/problems/problem-1/submissions?assignmentId=assignment-1",
      {
        method: "GET",
      },
    );
  ...
  it("returns 403 when access check fails", async () => {
    ...
    vi.mocked(assertExecutionAccess).mockRejectedValue(
      new ExecutionForbiddenError("Access denied"),
    );
