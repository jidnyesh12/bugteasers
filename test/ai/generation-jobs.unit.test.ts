...
describe("generation-jobs orchestration", () => {
  beforeEach(() => {
    ...
    normalizeOutputMock.mockImplementation((value: string) =>
      String(value).trim(),
    );
  });
      ...
      maybeSingleResults: [
        ...
        },
        {
          data: buildJobRow("validating", {
            progress_message:
              "Model code is ready. Now testing model answer on generated testcases.",
      ...
      maybeSingleResults: [
        ...
        },
        {
          data: buildJobRow("retrying", {
            progress_message:
              "Generation failed: AI service timeout. Retrying... (Attempt 2/4)",
  ...
  it("completes validating jobs when model outputs match testcase expectations", async () => {
    ...
    pistonExecuteMock.mockResolvedValueOnce({
      compile: null,
  ...
  it("dispatches targeted solution repair when last retry was a validation failure", async () => {
    ...
    const repairedProblem = {
      ...originalProblem,
      solution_code: "fixed cpp code",
    };
    const retryHistory: RetryHistoryEntry[] = [
      {
        attempt: 1,
        stage: "validating",
        error:
          "Model solution failed validation: Problem 1, test case 1: runtime error - SyntaxError",
        timestamp: "2026-01-01T00:00:01.000Z",
      },
    ];
  ...
  it("preserves result_payload in db when transitioning to retrying from a validation failure", async () => {
    ...
    const retryingUpdateCall = query.update.mock.calls.find(
      (call: unknown[]) => {
    ...
    const updatePayload = (retryingUpdateCall as unknown[])[0] as Record<
      string,
      unknown
    >;
