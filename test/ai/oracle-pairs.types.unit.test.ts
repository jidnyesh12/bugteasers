...
import type { OraclePair, TestCase, ModelAnswer, GenerationSeedInfo } from '../../lib/ai/oracle-pairs';
...
describe('GenerationSeed', () => {
  describe('Seed Derivation', () => {
    ...
    test('should produce different seeds for different problem IDs', () => {
      ...
      const seed1 = GenerationSeed.derive({ ...baseSeed, problemId: 'problem-1' });
      const seed2 = GenerationSeed.derive({ ...baseSeed, problemId: 'problem-2' });
  ...
  describe('Seed Verification', () => {
    ...
    test('should handle null/undefined gracefully', () => {
      ...
      expect(seed.verify(null as unknown as Partial<GenerationSeedInfo>)).toBe(false);
      expect(seed.verify(undefined as unknown as Partial<GenerationSeedInfo>)).toBe(false);
    });
  });
  ...
  describe('Seed Builder (Fluent API)', () => {
    ...
    test('should enforce required fields', () => {
      expect(() => {
        new SeedDerivationBuilder().withProblemId('problem').withTestCaseIndex(0).build();
      }).toThrow('masterSeed is required');

      expect(() => {
        new SeedDerivationBuilder().withMasterSeed('master').withTestCaseIndex(0).build();
      }).toThrow('problemId is required');

      expect(() => {
        new SeedDerivationBuilder().withMasterSeed('master').withProblemId('problem').build();
...
describe('OraclePair Factory', () => {
  ...
  test('should reject oracle pair with mismatched seeds', () => {
    ...
    expect(() => createOraclePair(testCase, wrongAnswer)).toThrow('Seed mismatch');
  });
...
describe('ConstraintRegistry', () => {
  ...
  test('should retrieve hard constraints', () => {
    ...
    expect(hardConstraints.every((c) => c.level === ConstraintLevel.HARD)).toBe(true);
  });
  ...
  test('should retrieve soft constraints', () => {
    ...
    expect(softConstraints.every((c) => c.level === ConstraintLevel.SOFT)).toBe(true);
  });
