...
describe("Bidirectional Validator", () => {
  ...
  describe("Hard Constraint Checking", () => {
    ...
    test("should fail with array_length_bounded violation", () => {
      ...
      expect(
        result.violations.some((c) => c.name === "array_length_bounded"),
      ).toBe(true);
    });
    ...
    test("should fail with no_nan_or_inf violation", () => {
      ...
      const result = validateHardConstraints(testCase);
      expect(result.valid).toBe(false);
      expect(result.violations.some((c) => c.name === "no_nan_or_inf")).toBe(
        true,
      );
    });
  ...
  describe("Oracle Pair Validation", () => {
    test("should validate consistent oracle pair", async () => {
      const testCase = createTestCase("5", "120", "seed-001", "resolved-001", {
        n: 5,
      });
      const modelAnswer = createModelAnswer(
        "code",
        "python",
        "seed-001",
        "test-model",
    ...
    test("should include diagnostics in result", async () => {
      ...
      expect(Array.isArray(result.diagnostics?.constraintViolations)).toBe(
        true,
      );
    ...
    test("should handle constraint violations in validation", async () => {
      ...
      const testCase = createTestCase("", "", "seed-001", "resolved-001", {
        arr: largeArray,
      });
      const modelAnswer = createModelAnswer(
        "code",
      ...
      expect(result.diagnostics?.constraintViolations.length).toBeGreaterThan(
        0,
      );
    });
