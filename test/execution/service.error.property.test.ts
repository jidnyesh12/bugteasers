...
function createSupabaseWithTestCases(testCases: Array<{
  id: string;
  input_data: string;
...
describe('Execution Service Error Handling Properties', () => {
  it('Property 19: runtime errors are captured and included in test results', async () => {
    await fc.assert(
      fc.asyncProperty(fc.string({ minLength: 1, maxLength: 250 }), async runtimeError => {
  ...
  it('invalid test case data should produce a descriptive validation error', async () => {
    await fc.assert(
      fc.asyncProperty(fc.string({ minLength: 1, maxLength: 64 }), async badValue => {
        const mockSupabase = createSupabaseWithTestCases([
          {
            id: 'test-1',
            input_data: badValue,
