...
describe('ExecutionService Unit Tests', () => {
  ...
  beforeEach(() => {
    ...
    mockEq = vi.fn().mockImplementation(() => {
      const chainable = {
        eq: mockEq,
        then: (resolve: (value: typeof mockEqResult) => void) => resolve(mockEqResult),
      };
      return chainable;
    });
  ...
  describe('runCode', () => {
    ...
    it('should handle database errors when fetching test cases', async () => {
      mockEq.mockImplementation(() => {
        const chainable = {
          eq: mockEq,
          then: (_resolve: unknown, reject: (error: Error) => void) => reject(new Error('Failed to fetch test cases: Database error')),
        };
        return chainable;
      });
  ...
  describe('Error Handling', () => {
    it('should propagate execution errors', async () => {
      vi.mocked(mockPistonClient.mapLanguage).mockReturnValue('python');
      vi.mocked(mockPistonClient.execute).mockRejectedValue(new Error('Execution timeout'));
    ...
    it('should handle compilation errors', async () => {
      ...
      const result = await service.runCode(
        { code: 'invalid java code', language: 'java', problemId: 'problem-1' }
      );
    ...
    it('should handle runtime errors', async () => {
      ...
      const result = await service.runCode(
        { code: 'print(x)', language: 'python', problemId: 'problem-1' }
      );
  ...
  describe('Edge Cases', () => {
    it('should handle partial success', async () => {
      ...
      mockEq.mockImplementation(() => {
        const chainable = {
          eq: mockEq,
          then: (resolve: (value: { data: TestCaseData[]; error: null }) => void) => resolve({
            data: [
              { id: 'test-1', input_data: 'input1', expected_output: 'output1', is_sample: true, points: 10 },
              { id: 'test-2', input_data: 'input2', expected_output: 'output2', is_sample: true, points: 10 },
            ],
            error: null,
          }),
        };
    ...
    it('should handle different programming languages', async () => {
      ...
      for (const { lang, pistonLang } of languages) {
        ...
        await service.runCode(
          { code: 'code', language: lang, problemId: 'problem-1' }
        );
