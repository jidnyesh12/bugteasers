    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async (code, language, userId, problemId) => {
          ...
          vi.mocked(mockPistonClient.mapLanguage).mockReturnValue(language === 'cpp' ? 'c++' : language);
          vi.mocked(mockPistonClient.execute).mockResolvedValue({
            language,
            version: '1.0',
            run: { stdout: 'output', stderr: '', code: 0, signal: null, output: 'output' },
    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async (code, language, userId, problemId) => {
          ...
          const hasSampleFilter = mockEqCalls.some(([key, value]) => key === 'is_sample' && value === true);
  ...
  beforeEach(() => {
    ...
    mockInsert = vi.fn().mockReturnValue({
      select: vi.fn().mockReturnValue({
        single: vi.fn().mockResolvedValue({ data: { id: 'submission-id' }, error: null }),
      }),
    });
    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async (code, language, userId, problemId) => {
          ...
          const hasSampleFilter = mockEqCalls.some(([key]) => key === 'is_sample');
...
describe('Compilation Errors Propagate to All Test Cases', () => {
  ...
  beforeEach(() => {
    ...
    const mockSelect = vi.fn().mockReturnValue({
      eq: vi.fn().mockResolvedValue({
        data: [
          { id: 'test-1', input_data: 'input1', expected_output: 'output1', is_sample: true, points: 10 },
          { id: 'test-2', input_data: 'input2', expected_output: 'output2', is_sample: false, points: 15 },
          { id: 'test-3', input_data: 'input3', expected_output: 'output3', is_sample: false, points: 20 },
        ],
        error: null,
      }),
    });
    ...
    fc.assert(
      fc.asyncProperty(
        ...
        async (code, language, userId, problemId) => {
          ...
          const result = await service.submitCode({ code, language, problemId }, userId);
        ...
        async (code, language, userId, problemId) => {
          ...
          vi.mocked(mockPistonClient.execute)
            .mockResolvedValueOnce({
              language,
              version: '1.0',
              run: { stdout: '', stderr: 'RuntimeError', code: 1, signal: null, output: 'RuntimeError' },
            } as ExecutionResponse)
            .mockResolvedValueOnce({
              language,
              version: '1.0',
              run: { stdout: 'output2', stderr: '', code: 0, signal: null, output: 'output2' },
            } as ExecutionResponse)
            .mockResolvedValueOnce({
              language,
              version: '1.0',
              run: { stdout: 'output3', stderr: '', code: 0, signal: null, output: 'output3' },
            } as ExecutionResponse);
