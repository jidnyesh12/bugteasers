...
export default function NewProblemPage() {
  ...
  const generationSteps = [
    { num: '1', label: 'Configure', key: 'form' as const },
    { num: '2', label: 'Validating', key: 'validating' as const },
    { num: '3', label: 'Preview & Edit', key: 'preview' as const },
  ];

  const currentStep: (typeof generationSteps)[number]['key'] =
    step === 'preview' ? 'preview' : isGenerating ? 'validating' : 'form';
  const currentStepIndex = generationSteps.findIndex((generationStep) => generationStep.key === currentStep);
  ...
  const handleCancel = () => {
    if (step === 'preview') { setStep('form'); setGeneratedProblems([]); setIsGenerating(false); }
    else { router.back(); }
  };
      ...
      <div className="flex items-center gap-3 mb-8">
        {generationSteps.map((s, i) => {
          ...
          <div key={s.key} className="flex items-center gap-3">
            {i > 0 && (
              <div className={`flex-1 h-px w-12 ${currentStepIndex >= i ? 'bg-[var(--accent-primary)]' : 'bg-[var(--border-primary)]'}`} />
            )}
            <div className="flex items-center gap-2">
              ...
              <span className={`text-sm font-semibold hidden sm:block ${isActive || isCompleted ? 'text-[var(--text-primary)]' : 'text-[var(--text-muted)]'}`}>
                {s.label}
              </span>
            </div>
          </div>
      ...
      <div className="bg-white border border-[var(--border-primary)] rounded-2xl p-6 lg:p-8">
        {step === 'form' ? (
          <ProblemGeneratorForm
            ...
            onGenerationFinish={handleGenerationFinish}
          />
        ) : (
