  ...
  try {
    ...
    if (session.user.role !== "instructor") {
      return NextResponse.json(
        { error: "Only instructors can assign assignments" },
        { status: 403 },
      );
    }
    ...
    if (error) {
      ...
      return NextResponse.json(
        { error: "Failed to assign to classrooms" },
        { status: 500 },
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (session.user.role !== "instructor") {
      return NextResponse.json(
        { error: "Only instructors can unassign assignments" },
        { status: 403 },
      );
    }
    ...
    if (classroomLookupError) {
      ...
      console.error(
        "Error validating classroom ownership:",
        classroomLookupError,
      );
      return NextResponse.json(
        { error: "Failed to validate classroom" },
        { status: 500 },
      );
    }
