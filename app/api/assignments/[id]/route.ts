...
export async function GET(
  ...
  try {
    ...
    if (!isInstructor && !isOwner) {
      ...
      if (accessError) {
        console.error(
          "Error validating assignment classroom mappings:",
          accessError,
        );
        return NextResponse.json(
          { error: "Failed to validate assignment access" },
          { status: 500 },
        );
      }
      ...
      for (const mapping of access as Array<{ classroom_id?: string | null }>) {
        ...
        if (enrollmentError) {
          console.error(
            "Error validating student enrollment for assignment access:",
            enrollmentError,
          );
          return NextResponse.json(
            { error: "Failed to validate assignment access" },
    ...
    const problems =
      assignment.assignment_problems
        ?.sort(
...
export async function PATCH(
  ...
  try {
    ...
    if (
      !existingAssignment ||
      existingAssignment.created_by !== session.user.id
    ) {
      return NextResponse.json(
        { error: "Assignment not found" },
        { status: 404 },
    ...
    if (closed_at !== undefined) {
      if (
        closed_at !== null &&
        (typeof closed_at !== "string" || Number.isNaN(Date.parse(closed_at)))
      ) {
        return NextResponse.json(
          { error: "Invalid closed_at value" },
          { status: 400 },
        );
      }

      updateData.closed_at =
        closed_at === null ? null : new Date(closed_at).toISOString();
    }
    ...
    if (problem_ids && Array.isArray(problem_ids)) {
      ...
      if (problem_ids.length > 0) {
        const { error: problemsError } = await supabase
          .from("assignment_problems")
          .insert(
            problem_ids.map((problem_id: string, index: number) => ({
              assignment_id: id,
              problem_id,
              order_index: index,
            })),
