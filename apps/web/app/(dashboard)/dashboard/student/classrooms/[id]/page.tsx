...
export default function StudentClassroomDetailsPage() {
  ...
  const { data: enrolledClassrooms = [], isFetching: isClassroomsLoading } =
    useQuery<{ id: string; classroom: ClassroomDetails }[]>({
      queryKey: queryKeys.classrooms.studentMine,
      queryFn: () =>
        fetchClassrooms<{ id: string; classroom: ClassroomDetails }>(),
  ...
  const classroom =
    enrolledClassrooms.find((item) => item.classroom?.id === params.id)
      ?.classroom ?? null;
  ...
  const formatDeadline = (deadline: string, closedAt?: string | null) => {
    ...
    const closedTimestamp =
      typeof closedAt === "string" ? Date.parse(closedAt) : Number.NaN;
    const isClosed =
      Number.isFinite(closedTimestamp) && closedTimestamp <= now.getTime();
    ...
    const closedFormatted = isClosed
      ? new Date(closedTimestamp).toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
          year: "numeric",
        })
      ...
      {assignments.length === 0 ? (
        ...
        <div className="grid gap-3">
          {assignments.map((assignment) => {
            const { formatted, isOverdue, isClosed, closedFormatted } =
              formatDeadline(assignment.deadline, assignment.closed_at);
            return (
              <div
                key={assignment.id}
                onClick={() =>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-xs text-[var(--text-muted)]">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span>
                          {assignment.problem_count || 0} problem
                          {(assignment.problem_count || 0) !== 1 ? "s" : ""}
                        </span>
                      </div>
                      <span>•</span>
                      <div className="flex items-center gap-1.5">
