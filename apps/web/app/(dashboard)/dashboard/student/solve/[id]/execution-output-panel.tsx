...
const MIN_OUTPUT_PANEL_HEIGHT =
  SOLVE_EDITOR_LAYOUT_BOUNDS.outputPanel.minHeightPx;
const MAX_OUTPUT_PANEL_HEIGHT =
  SOLVE_EDITOR_LAYOUT_BOUNDS.outputPanel.maxHeightPx;
const COLLAPSE_SNAP_HEIGHT =
  SOLVE_EDITOR_LAYOUT_BOUNDS.outputPanel.collapseSnapHeightPx;
const COLLAPSED_DOCK_HEIGHT =
  SOLVE_EDITOR_LAYOUT_BOUNDS.outputPanel.collapsedDockHeightPx;
const EXPAND_FROM_COLLAPSED_HEIGHT =
  SOLVE_EDITOR_LAYOUT_BOUNDS.outputPanel.expandFromCollapsedHeightPx;
...
const DEFAULT_OUTPUT_PANEL_HEIGHT =
  SOLVE_EDITOR_LAYOUT_BOUNDS.outputPanel.defaultHeightPx;

function clampOutputPanelHeight(value: number): number {
  return Math.min(
    MAX_OUTPUT_PANEL_HEIGHT,
    Math.max(MIN_OUTPUT_PANEL_HEIGHT, value),
  );
}
...
export function ExecutionOutputPanel({
  ...
  const inlineDiffParts = showDiffForCase
    ...
    : [];
  const actualDiffParts = inlineDiffParts.filter(
    (part) => part.kind !== "removed",
  );
  const expectedDiffParts = inlineDiffParts.filter(
    (part) => part.kind !== "added",
  );

  const [dragPreviewHeight, setDragPreviewHeight] = useState<number | null>(
    null,
  );
  const lastExpandedHeightRef = useRef(panelHeight);
  const resizeHandleRef = useRef<HTMLDivElement | null>(null);
  const headerRef = useRef<HTMLDivElement | null>(null);
  const [collapsedFloorHeight, setCollapsedFloorHeight] = useState<number>(
  ...
  const handleResizeStart = useCallback(
    (event: ReactPointerEvent<HTMLDivElement>) => {
      ...
      const onPointerMove = (moveEvent: PointerEvent) => {
        const nextRawHeight = roundToDragInterval(
  ...
  return (
    <div
      ...
      style={
        isCollapsed
          ? { height: `${collapsedFloorHeight}px` }
    ...
    >
      <div
        ...
        title={
          isCollapsed
            ? "Drag up to expand output panel"
      ...
      <div
        ref={headerRef}
        className={`flex items-center justify-between px-3 py-1.5 bg-[#252526] shrink-0 ${isCollapsed ? "" : "border-b border-[#3e3e42]"}`}
      >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            aria-label={
              isCollapsed ? "Expand output panel" : "Collapse output panel"
            }
            title={
              isCollapsed ? "Expand output panel" : "Collapse output panel"
            }
          ...
          {!executionResult ? (
            ...
            <>
              <div className="flex flex-wrap items-center gap-3">
                <span
                  className={`text-xl font-bold ${outputMeta.tone === "passed" ? "text-emerald-300" : outputMeta.tone === "failed" ? "text-rose-300" : outputMeta.tone === "partial" ? "text-amber-300" : outputMeta.tone === "error" ? "text-red-300" : "text-slate-300"}`}
                >
                  {outputMeta.title}
                </span>
                {scoreLine && (
                    ...
                    <div className="flex flex-wrap gap-2">
                      {caseRows.map((caseRow, index) => {
                        ...
                        return (
                          ...
                          >
                            <span
                              className={`inline-flex h-4 w-4 items-center justify-center rounded-full text-[10px] font-black ${statusMeta.iconClass}`}
                            >
                              {statusMeta.icon}
                            </span>
                      ...
                      <div className="space-y-2">
                        ...
                        {selectedCase.pointsText && (
                          <p className="text-xs text-gray-400">
                            Points: {selectedCase.pointsText}
                          </p>
                        )}
                      </div>
                    )}
              ...
              {executionResult.status !== "running" &&
                ...
                caseRows.length === 0 && (
                  <p className="text-sm text-gray-400">
                    No public testcase details to display for this submission.
                  </p>
                )}
