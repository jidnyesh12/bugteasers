...
export default function NewProblemPage() {
  ...
  const currentStepIndex = generationSteps.findIndex(
    (generationStep) => generationStep.key === currentStep,
  );
    ...
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-sm text-[var(--text-secondary)] mt-1">
          Use AI to generate coding problems with test cases, hints, and starter
          code
        </p>
      </div>
        ...
        {generationSteps.map((s, i) => {
          ...
          return (
            <div key={s.key} className="flex items-center gap-3">
              {i > 0 && (
                <div
                  className={`flex-1 h-px w-12 ${currentStepIndex >= i ? "bg-[var(--accent-primary)]" : "bg-[var(--border-primary)]"}`}
                />
              )}
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-sm font-semibold hidden sm:block ${isActive || isCompleted ? "text-[var(--text-primary)]" : "text-[var(--text-muted)]"}`}
                >
                  {s.label}
                </span>
              </div>
