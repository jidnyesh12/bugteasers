...
const LABEL_CLASS =
  "block text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)] mb-1.5";
...
export default function ProblemGeneratorForm({
  ...
  const { mutateAsync: generateProblemsAsync, isPending: internalLoading } =
    useMutation({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setPhase("generating");
      setGenerationStatusMessage(
        "Generation request queued. Preparing your problem draft...",
      );
    ...
    try {
      ...
      const rawMsg =
        err instanceof Error
          ? err.message
          : "An error occurred while clearing jobs";
      setError(rawMsg);
    } finally {
      setIsClearing(false);
    }
  ...
  const errorCategorySummary = retryHistory.reduce(
    (acc, entry) => {
      const key =
          ...
          <div className="retry-status-panel animate-slide-up">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm font-semibold text-amber-800">
                {isRetrying ? "Retry in Progress" : "Generation in Progress"}
              </span>
              {maxRetries > 0 && (
            ...
            {retryHistory.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <span
                    className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold border ${getStageBadgeColor(retryHistory[retryHistory.length - 1].stage)}`}
                  >
                    {getStageLabel(retryHistory[retryHistory.length - 1].stage)}
                  </span>
                  <span className="text-xs text-amber-700">
                    {retryHistory[retryHistory.length - 1].error.length > 100
                      ? retryHistory[retryHistory.length - 1].error.substring(
                          0,
                          100,
                        ) + "..."
                ...
                {retryHistory.length > 1 && (
                  ...
                  >
                    ...
                    {showRetryHistory ? "Hide" : "Show"} previous{" "}
                    {retryHistory.length - 1} attempt
                    {retryHistory.length > 2 ? "s" : ""}
                  </button>
                )}
                    ...
                    {retryHistory.slice(0, -1).map((entry, idx) => (
                      <div key={idx} className="retry-history-item">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span
                            className={`inline-flex items-center px-1.5 py-0 rounded text-[9px] font-bold border ${getStageBadgeColor(entry.stage)}`}
                          >
                            {getStageLabel(entry.stage)}
                          </span>
                        </div>
                        <p className="text-[10px] text-gray-500 leading-tight">
                          {entry.error.length > 150
                            ? entry.error.substring(0, 150) + "..."
                            : entry.error}
                        </p>
                      </div>
                    ))}
            ...
            <div className="mt-4 mb-2 flex flex-col gap-2.5">
              {(() => {
                ...
                const currentRank = backendStatus
                  ? (stageRank[backendStatus] ?? -1)
                  : -1;

                const steps = [
                  {
                    rank: 0,
                    activeLabel: "Preparing generation request...",
                    doneLabel: "Request queued",
                  },
                  {
                    rank: 1,
                    activeLabel: "AI drafting problem, testcases & solution...",
                    doneLabel: "Problem & solution generated",
                  },
                  {
                    rank: 2,
                    activeLabel: "Materializing test case templates...",
                    doneLabel: "Test cases materialized",
                  },
                  {
                    rank: 3,
                    activeLabel:
                      backendStatus === "retrying"
                        ? "Repairing solution code (retrying)..."
                        : "Running model answer on every test case...",
                    doneLabel: "All test cases validated",
                  },
                ];
                ...
                return steps.map((step, idx) => {
                  ...
                  const isActive =
                    idx <= 1
                      ...
                      : idx === 2
                        ? currentRank === 2 &&
                          !generationStatusMessage?.includes("test case")
                        : currentRank >= 2 &&
                          (generationStatusMessage?.includes("test case") ||
                            generationStatusMessage?.includes("Testing"));
                  const isDone =
                    idx <= 1
                      ? currentRank > step.rank
                      : idx === 2
                  ...
                  return (
                    <div
                      key={idx}
                      className={`flex items-center gap-3 transition-opacity duration-300 ${isActive ? "opacity-100" : isPending ? "opacity-35" : "opacity-65"}`}
                    >
                      ...
                      <span
                        className={`text-xs leading-tight ${isActive ? "text-amber-800 font-medium" : "text-gray-500"}`}
                      >
                        {isActive
                          ? step.activeLabel
                          : isDone
        ...
        <div className="p-4 rounded-xl bg-gray-50 border border-gray-200 animate-fade-in">
          <div className="flex items-center gap-2 mb-2">
            ...
            >
              ...
              <rect x="9" y="9" width="6" height="6" fill="#6B7280" />
            </svg>
      ...
      {(phase === "exhausted" || phase === "error") && !isLoading && (
        <div className="p-4 rounded-xl bg-red-50 border border-red-100 animate-fade-in">
          <div className="flex flex-col gap-3">
            ...
            {!hasRetryHistory ? (
              ...
              <>
                <p className="text-xs text-red-600">
                  Tried {retryHistory.length} times, but the AI could not
                ...
                {Object.entries(errorCategorySummary).length > 0 && (
                  <div className="space-y-1">
                    ...
                    {Object.entries(errorCategorySummary).map(
                      ([category, count]) => (
                        <div
                          key={category}
                ...
                <p className="text-[10px] text-red-400 mt-1">
                  💡 Try a different topic, simpler constraints, or a different
                  difficulty level.
                </p>
              </>
            )}
            ...
            {error &&
              error.includes("already have 2 active generation jobs") && (
                <button
                  type="button"
