...
export default function GeneratedProblemPreview({
  ...
  const solutionLanguage = detectSupportedLanguageFromCode(
  ...
  const removeExample = (index: number) => {
    updateProblem(
      "examples",
      (currentProblem.examples || []).filter((_, i) => i !== index),
    );
  };
  ...
  const removeHint = (index: number) => {
    updateProblem(
      "hints",
      currentProblem.hints.filter((_, i) => i !== index),
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-6 p-6 rounded-xl border border-[var(--border-primary)] bg-white">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          <span
            className={`flat-badge ${difficultyBadge[currentProblem.difficulty] || "flat-badge-blue"}`}
          >
          ...
          {editMode === "description" ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[400px]">
              <div className="h-full flex flex-col min-h-0">
                <label className="text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)] mb-1">
                  Markdown Input
                </label>
                <textarea
        ...
        <div>
          <div className="flex justify-between items-center mb-3">
            <label className={`${LABEL_CLASS} mb-0`}>
              Examples ({(currentProblem.examples || []).length})
            </label>
            <button
              onClick={addExample}
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <textarea
                      value={tc.input_data}
                      onChange={(e) =>
                        updateTestCase(i, "input_data", e.target.value)
                      }
