        ...
        if (testCase.input_template) {
          ...
          if (testCase.is_sample) {
            ...
            if (isValidOutput) {
              ...
              const exampleIndex =
                problem.test_cases.slice(0, i + 1).filter((tc) => tc.is_sample)
                  .length - 1;
...
export async function repairProblemSolutionCode(
  ...
  const repairedProblems: GeneratedProblem[] = existingProblems.map((p, i) =>
    i === failingProblemIndex
      ? { ...p, solution_code: parsed.solution_code }
      : p,
  );
...
function parseSolutionRepairResponse(text: string): { solution_code: string } {
  ...
  if (
    ...
    ((parsed as Record<string, unknown>).solution_code as string).trim()
      .length === 0
  ) {
    throw new Error(
      'Solution repair response missing valid "solution_code" field',
    );
  }

  return {
    solution_code: (parsed as Record<string, unknown>).solution_code as string,
  };
}

function parseGeneratedProblems(text: string): {
  ...
  try {
    ...
    throw new Error(
      `AI returned invalid JSON format: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
  ...
  for (let i = 0; i < problems.length; i++) {
    ...
    for (let j = 0; j < problem.test_cases.length; j++) {
      ...
      if (!hasTemplate && hasUnresolvedPlaceholder(inputData)) {
        throw new Error(
          "Test case input_data contains unresolved placeholders",
        );
      }
      ...
      if (!hasTemplate && hasUnresolvedPlaceholder(expectedOutput)) {
        throw new Error(
          "Test case expected_output contains unresolved placeholders",
        );
      }
