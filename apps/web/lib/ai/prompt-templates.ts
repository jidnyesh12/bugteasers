...
export function buildProblemGenerationPrompt(
  ...
  const difficultyGuidelines = {
    ...
    medium:
      "Intermediate level. Requires understanding of data structures, algorithms, and problem-solving strategies.",
  ...
  const lines = [
    ...
    "     CRITICAL: limit lengths to max 2000 to prevent execution buffer overflow! Dont give upto 10^5!",
...
export function buildRetryContextSection(
  ...
  if (
    allErrors.includes("json") ||
    allErrors.includes("parse") ||
    allErrors.includes("format")
  ) {
    lines.push("🔴 JSON FORMAT: Your response had JSON formatting issues.");
    lines.push(
      '   - Ensure your ENTIRE response is a single valid JSON object: {"problems": [...]}',
    );
    lines.push(
      "   - Do NOT include markdown code fences (```json) around the response",
    );
    lines.push(
      "   - Escape all backslashes in strings: use \\\\ for LaTeX commands",
    );
  ...
  if (
    ...
    allErrors.includes("syntax") ||
    allErrors.includes("runtime")
  ) {
    lines.push(
      "🔴 CODE ERRORS: Your solution code had compilation or runtime errors.",
    );
    ...
    lines.push(
      "   - Use explicit includes: <iostream>, <vector>, <algorithm>, <string>, <map>, <set>, etc.",
    );
    lines.push(
      "   - DO NOT include ANY comments (no // lines, no /* */ blocks) — they cause JSON escaping issues",
    );
    lines.push(
      '   - Remember: code is inside a JSON string, escape \\\\ and " properly',
    );
    lines.push("");
  }

  if (
    allErrors.includes("template") ||
    allErrors.includes("input_template") ||
    allErrors.includes("dsl")
  ) {
    lines.push("🔴 TEMPLATE DSL: Your input_template was invalid.");
    lines.push(
      "   - Every test case MUST have input_template with version, variables, and output",
    );
    lines.push("   - Variables must be defined before they are referenced");
    lines.push(
      '   - Use "const" type for fixed arrays, NOT "matrix" with "values"',
    );
    lines.push(
      "   - Valid types: const, int, choice, string, int_array, matrix, permutation, pairs, graph",
    );
    lines.push(
      '   - Output must reference defined variables via {"ref": "varName"}',
    );
    lines.push("");
  }

  if (
    allErrors.includes("mismatch") ||
    allErrors.includes("expected") ||
    allErrors.includes("output")
  ) {
    lines.push(
      "🔴 OUTPUT MISMATCH: Your solution produced wrong output for test cases.",
    );
    lines.push("   - Verify your algorithm is correct");
    lines.push(
      "   - Make sure output format matches what the problem description specifies",
    );
    lines.push(
      "   - Check edge cases: empty inputs, single elements, maximum values",
    );
    lines.push("");
  }
  ...
  if (allErrors.includes("validation") || allErrors.includes("missing")) {
    ...
    lines.push(
      "   - Every problem needs: title, description, difficulty, tags, constraints, examples, hints, solution_code, test_cases",
    );
    lines.push(
      "   - Every test case needs: input_template, is_sample (boolean), points (1-3)",
    );
  ...
  lines.push(
    "CRITICAL: Generate a COMPLETELY NEW and CORRECT response. Do NOT repeat previous mistakes.",
  );
  lines.push("");
...
export function buildOracleRepairPrompt(
  ...
  if (failure.errorType === "compile_error" && failure.stderr) {
    ...
    lines.push(
      "- Fix all compilation errors. Do NOT change the algorithm unless it is fundamentally broken.",
    );
    lines.push(
      "- FORBIDDEN: Do NOT use #include <bits/stdc++.h> — it causes compilation timeouts.",
    );
  ...
  if (failure.errorType === "model_answer_error" && failure.stderr) {
    ...
    lines.push("FIX INSTRUCTIONS:");
    lines.push(
      "- The code compiled but crashed at runtime (segfault, TLE, or non-zero exit).",
    );
    lines.push(
      "- Check for: out-of-bounds access, integer overflow, infinite loops, division by zero.",
    );
    lines.push("");
  }
  ...
  if (failure.errorType === "logic_consistency_error") {
    ...
    lines.push("FIX INSTRUCTIONS:");
    lines.push(
      "- The code compiled and ran, but produced WRONG output for the above input.",
    );
    lines.push(
      "- Your algorithm is logically incorrect. Fix the algorithm itself.",
    );
    lines.push(
      "- Verify edge cases: empty inputs, single elements, maximum constraint values.",
    );
    lines.push("");
  }
