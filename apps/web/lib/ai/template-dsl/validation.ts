...
const SUPPORTED_VARIABLE_TYPES: ReadonlySet<TestCaseTemplateVariable["type"]> =
  new Set([
    "const",
    "int",
    "choice",
...
const SUPPORTED_SORT_DIRECTIONS: ReadonlySet<SortDirection> = new Set([
  "asc",
  "desc",
]);
...
function assertVariableSpec(name: string, rawSpec: unknown): void {
  ...
  if (
    ...
    !SUPPORTED_VARIABLE_TYPES.has(
      variableType as TestCaseTemplateVariable["type"],
    )
  ) {
    throw new TemplateDslError(
      `Unsupported template variable type: ${String(variableType)}`,
    );
  }
  ...
  switch (variableType) {
    case "const": {
      if (!Object.prototype.hasOwnProperty.call(specRecord, "value")) {
        throw new TemplateDslError(
          `Variable "${name}" of type const must define value`,
        );
      }
    ...
    case "choice": {
      if (
        !Array.isArray(specRecord.values) ||
        specRecord.values.length === 0 ||
        !specRecord.values.every(isScalar)
      ) {
        throw new TemplateDslError(
          `variables.${name}.values must be a non-empty array of strings or numbers`,
        );
      }

      if (specRecord.weights !== undefined) {
        if (
          !Array.isArray(specRecord.weights) ||
          specRecord.weights.length !== specRecord.values.length
        ) {
          throw new TemplateDslError(
            `variables.${name}.weights length must match values length`,
          );
        }
        ...
        for (let index = 0; index < specRecord.weights.length; index += 1) {
          ...
          if (
            ...
            throw new TemplateDslError(
              `variables.${name}.weights[${index}] must be a positive finite number`,
            );
          }
        }
      }
    ...
    case "string": {
      ...
      if (specRecord.alphabet !== undefined) {
        if (
          typeof specRecord.alphabet !== "string" ||
          specRecord.alphabet.length === 0
        ) {
          throw new TemplateDslError(
            `variables.${name}.alphabet must be a non-empty string when provided`,
          );
        }
      }
      ...
      if (specRecord.charset !== undefined) {
        if (
          typeof specRecord.charset !== "string" ||
          !SUPPORTED_CHARSETS.has(specRecord.charset as CharsetName)
        ) {
          throw new TemplateDslError(
    ...
    case "int_array": {
      ...
      if (specRecord.sorted !== undefined) {
        if (
          typeof specRecord.sorted !== "string" ||
          !SUPPORTED_SORT_DIRECTIONS.has(specRecord.sorted as SortDirection)
        ) {
          throw new TemplateDslError(
    ...
    case "permutation": {
      ...
      if (
        specRecord.start !== undefined &&
        (!Number.isInteger(specRecord.start) ||
          !Number.isFinite(specRecord.start as number))
      ) {
        throw new TemplateDslError(
          `variables.${name}.start must be an integer when provided`,
        );
      }
      return;
    }
    ...
    case "graph": {
      ...
      assertOptionalBoolean(
        specRecord.connected,
        `variables.${name}.connected`,
      );
      assertOptionalBoolean(
        specRecord.allowSelfLoops,
        `variables.${name}.allowSelfLoops`,
      );
      assertOptionalBoolean(
        specRecord.allowMultiEdges,
        `variables.${name}.allowMultiEdges`,
      );
      assertOptionalBoolean(
        specRecord.oneIndexed,
        `variables.${name}.oneIndexed`,
      );
...
function validateOutputReferences(
  ...
  for (let index = 0; index < output.length; index += 1) {
    ...
    if (instruction.type === "line") {
      for (
        let valueIndex = 0;
        valueIndex < instruction.values.length;
        valueIndex += 1
      ) {
...
function validateOutputInstructionShape(
  instruction: unknown,
  index: number,
): void {
  ...
  if (
    ...
    throw new TemplateDslError(
      `output[${index}].type must be one of: line, lines, raw`,
    );
  }
  ...
  if (record.type === "line") {
    if (!Array.isArray(record.values) || record.values.length === 0) {
      throw new TemplateDslError(
        `output[${index}].values must be a non-empty array`,
      );
    }

    for (
      let valueIndex = 0;
      valueIndex < record.values.length;
      valueIndex += 1
    ) {
      ...
      if (!value || typeof value !== "object" || Array.isArray(value)) {
        throw new TemplateDslError(
          `output[${index}].values[${valueIndex}] must be an object`,
        );
      }

      const valueRecord = value as Record<string, unknown>;
      const hasRef = Object.prototype.hasOwnProperty.call(valueRecord, "ref");
      const hasLiteral = Object.prototype.hasOwnProperty.call(
        valueRecord,
        "literal",
      );
      ...
      if (hasRef) {
        if (
          typeof valueRecord.ref !== "string" ||
          (valueRecord.ref as string).trim().length === 0
        ) {
          throw new TemplateDslError(
            `output[${index}].values[${valueIndex}].ref must be a non-empty string`,
          );
        }

        if (
          valueRecord.join !== undefined &&
          typeof valueRecord.join !== "string"
        ) {
          throw new TemplateDslError(
            `output[${index}].values[${valueIndex}].join must be a string when provided`,
          );
        }
      }
      ...
      if (hasLiteral && !isScalar(valueRecord.literal)) {
        throw new TemplateDslError(
          `output[${index}].values[${valueIndex}].literal must be a string or number`,
        );
      }
    }

    if (
      record.separator !== undefined &&
      typeof record.separator !== "string"
    ) {
      throw new TemplateDslError(
        `output[${index}].separator must be a string when provided`,
      );
    }
  ...
  if (record.type === "lines") {
    if (typeof record.from !== "string" || record.from.trim().length === 0) {
      throw new TemplateDslError(
        `output[${index}].from must be a non-empty string`,
      );
    }
  ...
  if (typeof record.value !== "string") {
    throw new TemplateDslError(
      `output[${index}].value must be a string for raw instructions`,
    );
  }
}

export function validateTestCaseInputTemplate(
  template: unknown,
): asserts template is TestCaseInputTemplate {
  ...
  if (
    ...
    (!Number.isInteger(candidate.version) ||
      candidate.version !== TEMPLATE_DSL_VERSION)
  ) {
    throw new TemplateDslError(
  ...
  if (candidate.seed !== undefined && typeof candidate.seed !== "string") {
    throw new TemplateDslError(
      "input_template.seed must be a string when provided",
    );
  }
  ...
  if (Object.keys(variables).length === 0) {
    throw new TemplateDslError(
      "input_template.variables must define at least one variable",
    );
  }
  ...
  if (!Array.isArray(output) || output.length === 0) {
    throw new TemplateDslError(
      "input_template.output must be a non-empty array",
    );
  }
